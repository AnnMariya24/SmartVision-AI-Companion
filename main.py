...
is_listening = False   # ⭐ NEW (prevents self-talking loop)
current_objects = []
...
# 🎤 LISTENING THREAD (Conversation Mode)
def listen_worker():
    global assistant_active, assistant_state

    WAKE_WORD = "vision"

    while True:

        # Assistant only waits for wake word
        ...
        if WAKE_WORD in command:
            ...
            # 🗣️ Always speak reply (even if OpenAI fails)
...
def speak_smalltalk():
    ...
    speech_queue.put(random.choice(messages))
...
while True:
    ...
    elif (not is_listening) and current_time - last_spoken_time > 7 and final_message:
        speech_queue.put(final_message)
        last_spoken_time = current_time

    # 💬 SMALL TALK WHEN SAFE
    if (not is_listening) and current_time - last_smalltalk_time > SMALLTALK_INTERVAL and not safety_message:
        speak_smalltalk()
    ...
    time.sleep(0.12) # ⭐ Reduce CPU usage
